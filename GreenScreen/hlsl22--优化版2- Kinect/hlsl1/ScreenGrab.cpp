...
#pragma pack(pop)
		...
		ComPtr<ID3D11Texture2D>& pStaging)
	{
		...
		else
		{
			...
			pContext->CopyResource(pStaging.Get(), pSource);/*复制原纹理资源到新的纹理资源*/
		}
...
HRESULT DirectX::SaveWICTextureToFile(
	ID3D11DeviceContext* pContext,/*设备上下文*/
	ID3D11Resource* pSource,/*纹理资源*/
	REFGUID guidContainerFormat,/*保存的文件格式*/
	const wchar_t* fileName,/*要保存的地址*/
	...
	std::function<void(IPropertyBag2*)> setCustomProps)
{
	...
	D3D11_TEXTURE2D_DESC desc = {};
	ComPtr<ID3D11Texture2D> pStaging;
	HRESULT hr = CaptureTexture(pContext, pSource, desc, pStaging);/*复制一份纹理资源到pStaging*/
	if (FAILED(hr))
		return hr;

	// Determine source format's WIC equivalent确定源格式的WIC等效项
	...
	else
	{
		...
		default:
			//targetGuid = GUID_WICPixelFormat24bppBGR;
			targetGuid = GUID_WICPixelFormat32bppRGBA;/*我自己改的*/
			break;
		}
	}
	//targetGuid = GUID_WICPixelFormat32bppRGBA;/*我自己加的，目标类型设置为RGBA*/
